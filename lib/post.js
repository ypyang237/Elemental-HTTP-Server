...
var templateCreator = require('../templates/templateCreator');
...
function postRequest(url, request, response) {
  ...
  request.on('data', function(chunk) {
    ...
    var options = {
      ...
      elementSymbol: parsed.elementSymbol,
      elementAtomicNumber: parsed.elementAtomicNumber,
      elementDescription: parsed.elementDescription,
      path: ('./public/' + elementName + '.html')
    };
    ...
    fs.stat(path, function(err, stats) {
      ...
      if(err) {
        ...
        templateCreator(options);
//Stopped here, could i use the line above to replace the lines below?
